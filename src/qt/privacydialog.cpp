...
#include "zblibrarycontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zblib ought to be enough for anybody." - Bill Gates, 2017
    ui->zblibpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzblibSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzblib->setEnabled(false);
        ui->pushButtonMintzblib->setToolTip(tr("zblib is currently disabled due to maintenance."));

        ui->pushButtonSpendzblib->setEnabled(false);
        ui->pushButtonSpendzblib->setToolTip(tr("zblib is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zblibpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzblib_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zblib is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zblib...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zblib in ") +
...
void PrivacyDialog::on_pushButtonSpendzblib_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zblib is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zblib
        sendzblib();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zblib
    sendzblib();
}

void PrivacyDialog::on_pushButtonZblibraryControl_clicked()
{
    ZblibraryControlDialog* zblibControl = new ZblibraryControlDialog(this);
    zblibControl->setModel(walletModel);
    zblibControl->exec();
}

void PrivacyDialog::setZblibraryControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzblibSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzblib()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid blibrary Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zblibpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zblib is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " blibrary </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zblib</b>";
    ...
    // use mints from zblib selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZblibraryControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZblibraryControlDialog::GetSelectedMints();
    }
    ...
    // Clear zblibrary selector in case it was used
    ZblibraryControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zblib Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " blibrary, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zblib Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zblib </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zblib "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zblib "));
    ui->labelzblibAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzblibSyncStatus->setVisible(fShow);
}
