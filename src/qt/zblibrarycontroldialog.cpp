...
#include "ui_zblibrarycontroldialog.h"
...
std::list<std::string> ZblibraryControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZblibraryControlDialog::listMints;

ZblibraryControlDialog::ZblibraryControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZblibraryControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZblibraryControlDialog::~ZblibraryControlDialog()
{
    delete ui;
}

void ZblibraryControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZblibraryControlDialog::updateList()
{
...
void ZblibraryControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZblibraryControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZblibrary_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZblibraryControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZblibraryControlDialog::GetSelectedMints()
{
...
void ZblibraryControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
